...
public class UserDao {
	...
	public boolean saveStudent(Student student) {
		String query="insert into user(enrollmentno, name, address, course, fees) values (?,?,?,?,?)";
		boolean check=false;
		try {
	...
	public boolean updateStudent(Student st,int id) {
		...
		PreparedStatement psmt=con.prepareStatement("update user set enrollmentno=?,name=?,address=?,course=?,fees=? where id=?");
	...
	public Student UserByNameAndEnrollmentNo(String name,String EnrollmentNo){
		Student s=new Student();
		try {
			Statement st=con.createStatement();
			ResultSet rs = st.executeQuery("select * from user where name='" + name + "' and enrollmentno='" + EnrollmentNo + "'");
			if(rs.next()) {
